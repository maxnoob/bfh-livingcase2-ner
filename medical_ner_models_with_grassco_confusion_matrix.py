...
"""
        Confusion matrix for medical NER not reasonable, since only DATE entity is both in the model and the annotated text.
        TODO: find model and/or corpus with corresponding entities.
    """

import json

# Load the JSON file
json_file_name = "Clausthal.txt_phi.json"
with open(f"./Datasets/GraSSCo/annotation/grascco_phi_annotation_json/{json_file_name}", 'r', encoding='utf-8') as json_file:
    json_data = json.load(json_file)

# Extract entities from the JSON
annotated_entities = set()
for feature in json_data.get("%FEATURE_STRUCTURES", []):
    if feature.get("%TYPE") == "webanno.custom.PHI" and "kind" in feature:
        annotated_entities.add(feature["kind"])

# Display the extracted entities
print("\n[bold underline]Annotated Entities:[/bold underline]\n%s" % annotated_entities)

# Read text file
text_file_name = "Clausthal.txt"
with open(f"./Datasets/GraSSCo/corpus/{text_file_name}", "r", encoding="utf-8") as text_file:
    text = text_file.read()
